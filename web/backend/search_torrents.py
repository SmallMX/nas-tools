...
def search_media_by_message(input_str, in_from: SearchType, user_id, user_name=None):
    ...
    # 接收到文本，开始查询可能的媒体信息供选择
        ...
        is_download = input_str.startswith("http") or input_str.startswith("magnet:")
        if not is_download:
...
def __search_media(in_from, media_info, user_id, user_name=None):
    ...
    开始搜索和发送消息
    """
    download_requirements, messages = Downloader().build_download_requirements(media_info=media_info)
    if messages:
        Message().send_channel_msg(channel=in_from,
                                   title="\n".join(messages),
                                   user_id=user_id)
    ...
    Message().send_channel_msg(channel=in_from,
                               ...
                               user_id=user_id)
    _, _, search_count, download_count = Searcher().search_one_media(
        media_info=media_info,
        in_from=in_from,
        no_exists=download_requirements,
