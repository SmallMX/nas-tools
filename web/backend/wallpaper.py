...
def get_login_wallpaper(today=None):
    """按调用当天获取壁纸；日期参数仅用于确定性测试和兼容旧调用。"""
    date_key = today or datetime.datetime.now().strftime('%Y%m%d')
    return _get_login_wallpaper_for_date(date_key)


@lru_cache(maxsize=2)
def _get_login_wallpaper_for_date(today):
    """
    获取Base64编码的壁纸图片
    """
    ...
    if img_url:
        try:
            res = RequestUtils(timeout=5).get_res(img_url)
