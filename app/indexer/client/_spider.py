...
class TorrentSpider(feapder.AirSpider):
    ...
    def start_requests(self):
        ...
        if self.keyword:
            ...
            if indexer_params:
                ...
                if search_area and (not search_word or not search_word.startswith("tt")):
        ...
        log.info(f"【Spider】开始请求：{StringUtils.redact_url(searchurl)}")
    ...
    def Getdownload(self, torrent):
        ...
        if "detail" in self.fields.get('download', {}):
            ...
            if items:
                download_url = self.__filter_text(items[0], self.fields.get('download', {}).get('filters'))
                if not download_url:
                    return
                if not download_url.startswith("http") and not download_url.startswith("magnet"):
                    self.torrents_info['enclosure'] = self.domain + download_url[1:] if download_url.startswith(
                        "/") else self.domain + download_url
                else:
                    self.torrents_info['enclosure'] = download_url
    ...
    @staticmethod
    def __selector_value(torrent, selector):
        item = torrent(selector.get('selector', selector.get('selectors', ''))).clone()
        if 'remove' in selector:
            for remove_selector in selector.get('remove', '').split(', '):
                item.remove(remove_selector)
        if 'attribute' in selector:
            values = [node.attr(selector.get('attribute')) for node in item.items() if node]
        else:
            values = [node.text() for node in item.items() if node]
        if not values:
            return ''
        if 'contents' in selector:
            contents = values[0].split('\n')
            index = int(selector.get('contents'))
            return contents[index] if -len(contents) <= index < len(contents) else ''
        if 'index' in selector:
            index = int(selector.get('index'))
            return values[index] if -len(values) <= index < len(values) else ''
        return values[0]

    def Getdate(self, torrent):
        """
        合并 date_added/date_elapsed，兼容新版站点配置中的 date 模板。
        """
        selector = self.fields.get('date', {})
        if not selector:
            return
        if selector.get('selector') or selector.get('selectors'):
            value = self.__selector_value(torrent, selector)
        elif selector.get('text'):
            value = Template(selector.get('text')).render(fields={
                'date_added': self.torrents_info.get('pubdate'),
                'date_elapsed': self.torrents_info.get('date_elapsed')
            })
        else:
            return
        self.torrents_info['pubdate'] = self.__filter_text(value, selector.get('filters'))
    ...
    def Getfree_deadline(self, torrent):
        ...
        if not value and selector.get('default_value'):
            value = Template(selector.get('default_value')).render(
                fields=self.torrents_info,
                max_time=str(datetime.datetime.max)
            )
            if value and selector.get('default_value_format'):
                value = datetime.datetime.strptime(value, selector.get('default_value_format'))
        self.torrents_info['freedate'] = value or None
    ...
    def Getcategory(self, torrent):
        ...
        tv_categories = [str(item.get('id')) for item in self.category.get('tv') or []]
        movie_categories = [str(item.get('id')) for item in self.category.get('movie') or []]
        if value in tv_categories and value not in movie_categories:
            self.torrents_info['category'] = MediaType.TV.value
        elif value in movie_categories:
        ...
        for filter_item in filters:
            ...
            try:
                ...
                if method_name == "re_search" and isinstance(args, list):
                    match = re.search(r"%s" % args[0], text)
                    text = match.group(args[-1]) if match else ''
                    ...
                    text = text.lstrip(args[0] if isinstance(args, list) and args else args)
                elif method_name == "appendleft":
                    text = f"{args}{text}"
                elif method_name == "querystring":
                    values = parse_qs(urlparse(text).query).get(args)
                    if values:
                        text = values[0]
                    else:
                        value_match = re.search(rf"{re.escape(str(args))}=([^&]+)", text)
                        text = value_match.group(1) if value_match else ''
                elif method_name == "date_en_elapsed_parse":
                    text = StringUtils.get_time_stamp(text)
                elif method_name == "date_elapsed_parse":
                    days = re.search(r'(\d+)日', text)
                    hours = re.search(r'(\d+)時', text)
                    minutes = re.search(r'(\d+)分', text)
                    text = datetime.datetime.now() + datetime.timedelta(
