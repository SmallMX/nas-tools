...
from app.indexer.client._api_spider import (HDDolbySpider, HaiDanSpider,
                                            MTorrentSpider, RousiSpider,
                                            YemaSpider)
...
class BuiltinIndexer(_IIndexClient):
    ...
    _api_spiders = {
        ...
        "YemaSpider": YemaSpider,
        "HaiDanSpider": HaiDanSpider,
        "HDDolbySpider": HDDolbySpider,
        "RousiSpider": RousiSpider
    }
    ...
    def get_indexers(self, check=True, public=False, indexer_id=None):
        ...
        for site in Sites().get_sites():
            if not site.get("rssurl") and not site.get("signurl"):
                continue
            if not site.get("cookie") and not site.get("apikey"):
                continue
    ...
    def search(self, order_seq,
        ...
        try:
            ...
            elif indexer.parser in self._api_spiders:
                result_array = self._api_spiders[indexer.parser](indexer=indexer).search(
                    keyword=search_word,
    ...
    def list(self, index_id, page=0, keyword=None):
        ...
        if indexer.parser == "RenderSpider":
            ...
            return self._api_spiders[indexer.parser](indexer=indexer).search(
                keyword=keyword,
                page=page)
        elif indexer.parser == "TNodeSpider":
