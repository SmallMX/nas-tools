...
class Fanart:
    ...
    def __get_fanart_images(self, media_type, queryid):
        if not media_type or not queryid:
            return
        api_key = Config().get_config('app').get('fanart_api_key')
        if not api_key:
            return
        try:
            ret = self.__request_fanart(media_type=media_type, queryid=queryid, api_key=api_key)
    ...
    def __request_fanart(cls, media_type, queryid, api_key):
        if media_type == MediaType.MOVIE:
            image_url = FANART_MOVIE_API_URL % queryid
        else:
            image_url = FANART_TV_API_URL % queryid
        try:
            return RequestUtils(proxies=cls._proxies, timeout=5).get_res(
                image_url, params={'api_key': api_key}
            )
