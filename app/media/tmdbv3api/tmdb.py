...
class TMDb(object):
    ...
    def proxies(self, proxies):
        ...
        os.environ[self.TMDB_PROXIES] = jsonlib.dumps(safe_proxies)
    ...
    def _parse_proxies(proxies):
        if not proxies:
            return None
        try:
            parsed = jsonlib.loads(proxies)
        except (TypeError, ValueError, jsonlib.JSONDecodeError) as error:
            raise TMDbException("Invalid proxy configuration.") from error
    ...
    def cached_request(method, url, data, proxies):
        return requests.request(
            ...
            url,
            data=data,
            proxies=TMDb._parse_proxies(proxies),
            verify=True,
            timeout=10,
        )
    ...
    def _call(
        ...
        if self.cache and self.obj_cached and call_cached and method != "POST":
            ...
            req = self._session.request(
                ...
                url,
                data=data,
                proxies=self._parse_proxies(self.proxies),
                timeout=10,
                verify=True,
            )
