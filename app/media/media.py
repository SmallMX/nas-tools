...
from app.media.tmdbv3api import TMDb, Search, Movie, TV, Person, Find, TMDbException, Discover, Trending, Genre
from app.utils import RequestUtils, StringUtils, cacheman
...
class Media:
    ...
    @staticmethod
    def _get_tmdb_domain(app, laboratory):
        tmdb_proxy = laboratory.get('tmdb_proxy') if isinstance(laboratory, dict) else None
        if isinstance(tmdb_proxy, str) and tmdb_proxy.strip():
            return tmdb_proxy.strip().rstrip('/')
        return app.get("tmdb_domain")
    ...
    def init_config(self):
        ...
        if app:
            if app.get('rmt_tmdbkey'):
                self.tmdb = TMDb()
                self.tmdb.domain = self._get_tmdb_domain(app, laboratory)
