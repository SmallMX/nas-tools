...
class DoubanApi(object):
    ...
    @classmethod
    def __sign(cls, url: str, ts: int, api_secret_key: str, method='GET') -> str:
        url_path = parse.urlparse(url).path
        raw_sign = '&'.join([method.upper(), parse.quote(url_path, safe=''), str(ts)])
        return base64.b64encode(hmac.new(api_secret_key.encode(), raw_sign.encode(), hashlib.sha1).digest()
                                ).decode()

    @classmethod
    def __invoke(cls, url, **kwargs):
        app_config = Config().get_config('app')
        api_key = app_config.get('douban_api_key')
        api_secret_key = app_config.get('douban_api_secret')
        if not api_key or not api_secret_key:
            return {}
        return cls.__invoke_with_credentials(url, api_key, api_secret_key, **kwargs)

    @classmethod
    @lru_cache(maxsize=256)
    def __invoke_with_credentials(cls, url, api_key, api_secret_key, **kwargs):
        ...
        params.update({
            ...
            'apiKey': api_key,
            '_ts': ts,
            '_sig': cls.__sign(url=req_url, ts=ts, api_secret_key=api_secret_key)
        })
