...
class Downloader:
    clients = {}
    _downloader_schema = (Qbittorrent, Transmission)
    ...
    def init_config(self):
        ...
        self._default_client_type = None
        self._pt_monitor_only = False
        ...
        # 配置热更新后必须按新配置重建客户端，旧对象仅供已在执行的调用继续使用
        with client_lock:
            self.clients = {}
        # 下载目录配置
    ...
    def __get_client(self, ctype: DownloaderType, conf: dict = None):
        if not ctype:
            return None
        if not isinstance(ctype, DownloaderType):
            ctype = self.__get_client_type(ctype)
        if not ctype:
            return None
        with client_lock:
    ...
    def download(self,
        ...
        # 默认值
        _xpath, _hash, site_info, retmsg = None, False, {}, ""
        # 有种子文件时解析种子信息
        if torrent_file:
        ...
        # 没有种子文件解析链接
            ...
            if url.startswith("magnet:"):
                ...
                # [XPATH]为需从详情页面解析磁力链
                if url.startswith("[") and not Torrent.is_dynamic_download_url(url):
                    _xpath = url[1:-1]
                    url = page_url
                ...
                # 从HTTP链接下载种子
                    ...
                    # 获取Cookie和ua等
                    site_info = self.sites.get_site_attr(
                        page_url if Torrent.is_dynamic_download_url(url) else url)
                    # 下载种子文件，并读取信息
        ...
        try:
            ...
            print_url = StringUtils.redact_url(print_url)
            if is_paused:
    ...
    def get_remove_torrents(self, downloader=None, config=None):
        ...
        if not _client:
            return []
        remove_config = dict(config)
        filter_tags = remove_config.get("tags") or []
        if isinstance(filter_tags, str):
            filter_tags = [tag for tag in filter_tags.split(";") if tag]
        else:
            filter_tags = list(filter_tags)
        if (self._pt_monitor_only or remove_config.get("onlynastool")) and PT_TAG not in filter_tags:
            filter_tags.append(PT_TAG)
        remove_config["filter_tags"] = filter_tags
        torrents = _client.get_remove_torrents(config=remove_config)
    ...
    def get_torrents(self, torrent_ids):
        ...
        if not client:
            return None, []
        torrent_list, _ = client.get_torrents(ids=torrent_ids)
        return client_type, torrent_list
    ...
    def start_torrents(self, downloader=None, ids=None):
        ...
        if not downloader:
            ...
            if not client:
                return False
            return client.start_torrents(ids)
        else:
            _client = self.__get_client(downloader)
            if not _client:
                return False
    ...
    def stop_torrents(self, downloader=None, ids=None):
        ...
        if not downloader:
            ...
            if not client:
                return False
            return client.stop_torrents(ids)
        else:
            _client = self.__get_client(downloader)
            if not _client:
                return False
    ...
    def delete_torrents(self, downloader=None, ids=None, delete_file=False):
        ...
        if not downloader:
            ...
            if not client:
                return False
            return client.delete_torrents(delete_file=delete_file, ids=ids)
        else:
            _client = self.__get_client(downloader)
            if not _client:
                return False
    ...
    def build_download_requirements(self, media_info, requirements=None, episode_counts=None):
        """
        将媒体请求转换为待下载的季集范围。

        电影不需要季集范围；电视剧和动漫按请求季集或TMDB季信息构造。
        :param media_info: 已识别的媒体信息，包括标题、年份、季、集信息
        :param requirements: 已构造的下载季集范围，有传入时继续合并
        :param episode_counts: 各季的总集数覆盖值
        :return: 下载季集范围、提示消息
        """
        if requirements is None:
            requirements = {}
        if not media_info or media_info.type == MediaType.MOVIE:
            return requirements, []

        if episode_counts is None:
            episode_counts = {}
        requested_seasons = media_info.get_season_list() if media_info.begin_season else []
        requested_episodes = sorted(set(media_info.get_episode_list() or []))
        if requested_episodes and not requested_seasons:
            requested_seasons = [1]

        tv_info = media_info.tmdb_info or {}
        if not tv_info.get("seasons") and media_info.tmdb_id:
            detail_info = self.media.get_tmdb_info(mtype=MediaType.TV, tmdbid=media_info.tmdb_id) or {}
            if detail_info:
                tv_info = detail_info

        season_details = {
            int(item.get("season_number")): int(item.get("episode_count") or 0)
            for item in self.media.get_tmdb_tv_seasons(tv_info)
            if item.get("season_number") and int(item.get("season_number")) > 0
        }
        if not requested_seasons:
            requested_seasons = sorted(season_details) or [1]

        pending_items = requirements.setdefault(media_info.tmdb_id, [])
        for season in requested_seasons:
            season = int(season)
            episode_count = int(episode_counts.get(season) or season_details.get(season) or 0)
            episodes = requested_episodes if len(requested_seasons) == 1 else []
            if episodes and not episode_count:
            ...
            if pending_item not in pending_items:
                pending_items.append(pending_item)

        log.info("【Downloader】按请求内容下载：%s %s" % (
