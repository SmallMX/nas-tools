...
# ===== 3. 核心业务：刷流（自动化下载与删种） =====
# 初学者注意：刷流（Brush）是 PT（Private Tracker）圈的行话，意思是自动下载免费的种子，挂机上传赚取积分或上传量，然后再自动删除。
# 这个类是整个刷流功能的大脑，负责定时去检查有没有新种子，以及检查有没有种子满足条件可以删除了。

@singleton # 单例模式，确保整个程序运行期间只有一个 BrushTask 实例在干活
class BrushTask(object):
    ...
    def init_config(self):
        """
        初始化刷流配置。
        初学者注意：当系统启动，或者用户在网页上修改了刷流配置后，都会调用这个方法重新排班。
        """
        ...
        if not self._brush_tasks:
            return
        # 启动RSS任务（去各大网站的RSS订阅源拉取最新发布的种子）
        task_flag = False
        self._scheduler = BackgroundScheduler(timezone=Config().get_timezone())
        self._scheduler.add_listener(self.__remove_job_db_session,
                                     EVENT_JOB_EXECUTED | EVENT_JOB_ERROR)
        # 初学者注意：这里遍历所有开启的刷流任务，为它们设定定时执行器（比如每 10 分钟执行一次 check_task_rss）
        ...
        # 启动删种任务
        if task_flag:
            # 如果有刷流任务，就顺便启动一个“打扫战场”的定时任务
            # 每隔一段时间（BRUSH_REMOVE_TORRENTS_INTERVAL）去下载器里看看，有没有达到分享率或者做种时间的种子，把它们删掉
    ...
    def get_brushtask_info(self, taskid=None):
        ...
        for task in brushtasks:
            try:
                ...
                log.error("【Brush】刷流任务 %s 规则解析失败，已跳过：%s" % (task.name, str(err)))
                continue
            site_info = self.sites.get_sites(siteid=task.site) or {}
            if site_info:
    ...
    def check_task_rss(self, taskid):
        """
        【核心方法 1】检查RSS并添加下载，由定时服务定期调用。
        初学者注意：这个方法就像一个采购员。拿着清单（RSS地址），看看有没有符合条件（免费、体积合适、做种人数少）的新鲜货（种子），有就买（扔给下载器）。
        ...
        rss_result = RssParser.parse_rssxml(rss_url)
        if len(rss_result) == 0:
    ...
    def remove_tasks_torrents(self):
        """
        【核心方法 2】打扫战场：检查所有任务下载完成的种子，按条件进行删除，并更新任务数据。
        由定时服务调用。
        初学者注意：这里会去联系 qBittorrent 或 Transmission，查询当前所有种子的状态（比如已经上传了多少，做种多久了）。
        如果发现“指标达标”了，就会指挥下载器把这个种子连同文件一起删掉，释放硬盘空间。
        """
        ...
        for taskinfo in self._brush_tasks:
            ...
            try:
                ...
                if client_type == self._qb_client:
                    ...
                    existing_torrents, has_err = downloader.get_torrents(ids=torrent_ids)
                    if has_err:
                        log.warn("【BRUSH】任务 %s 获取种子存在状态失败" % task_name)
                        continue
                    existing_ids = {torrent.get("hash") for torrent in existing_torrents}
                    remove_torrent_ids = list(set(torrent_ids).difference(existing_ids))
                    # 检查完成状态的
                    ...
                    for torrent in torrents:
                        ...
                        # 已开始时间 秒
                        dltime = max(1, int(time.time() - torrent.get("added_on")))
                        # 已做种时间 秒
                        ...
                        if need_delete:
                            ...
                            if torrent_id not in delete_ids:
                                ...
                                if sendmessage:
                                    delete_messages.append((delete_type, torrent.get('name')))
                    # 检查下载中状态的
                ...
                # transmission
                    ...
                    if not torrent_ids:
                        log.warn("【BRUSH】任务 %s 没有有效的 Transmission 种子ID" % task_name)
                        continue
                    ...
                    if has_err:
                        log.warn("【BRUSH】任务 %s 获取种子存在状态失败" % task_name)
                        continue
                    existing_ids = {torrent.id for torrent in existing_torrents}
                    remove_torrent_ids = list(set(torrent_ids).difference(existing_ids))
                    # 检查完成状态
                    ...
                    for torrent in torrents:
                        ...
                        date_now = int(time.mktime(datetime.now().timetuple()))
                        dltime = max(1, date_now - int(time.mktime(torrent.date_added.timetuple())))
                        ...
                        if need_delete:
                            ...
                            if torrent_id not in delete_ids:
                                ...
                                update_torrents.append(("%s,%s" % (uploaded, downloaded), taskid, torrent_id))
                                if sendmessage:
                                    delete_messages.append((delete_type, torrent.name))
                    # 检查下载状态
                    ...
                    for torrent in torrents:
                        ...
                        torrent_id = torrent.id
                        # 下载耗时
                        dltime = max(1, int((datetime.now().astimezone() - torrent.date_added).total_seconds()))
                        # 下载量
                        downloaded = int(torrent.total_size * torrent.progress / 100)
                        total_downloaded += downloaded
                        # 上传量
                ...
                deleted_count = 0
                if delete_ids:
                    if downloader.delete_torrents(delete_file=True, ids=delete_ids):
                        deleted_count = len(delete_ids)
                        if update_torrents and not self.dbhelper.update_brushtask_torrent_state(update_torrents):
                            log.error("【Brush】任务 %s 删除成功，但数据库状态更新失败" % task_name)
                        for delete_type, torrent_name in delete_messages:
                            __send_message(task_name, delete_type, torrent_name)
                        log.info("【Brush】任务 %s 共删除 %s 个刷流下载任务" % (task_name, deleted_count))
                    else:
                        log.error("【Brush】任务 %s 调用下载器删除失败，保留数据库任务以便重试" % task_name)
                else:
                ...
                self.dbhelper.add_brushtask_upload_count(brush_id=taskid,
                                                         ...
                                                         remove_count=deleted_count + len(remove_torrent_ids))
    ...
    def __download_torrent(self,
        ...
        _, content, _, _, retmsg = Torrent().get_torrent_info(
            ...
            source_url=site_info.get("strict_url"))
        if content:
            # 添加下载
            if downloadercfg.get("type") == self._qb_client:
                # 初始化下载器
        ...
        if not download_id:
            ...
            log.warn(f"【Brush】{taskname} 添加下载任务出错：{title}，"
                     ...
                     f"种子链接：{StringUtils.redact_url(enclosure)}")
            return False
        else:
            # 下载成功
    ...
    def __check_rss_rule(self,
        ...
        try:
            ...
            if rss_rule.get("peercount"):
                ...
                peer_counts = peercount_str.split("#", 1)
            ...
            if rss_rule.get("pubdate") and pubdate:
                ...
                if len(rule_pubdates) >= 2 and rule_pubdates[1]:
                    ...
                    if (localnowtime - localpubdate).total_seconds() / 3600 > float(rule_pubdates[1]):
                        log.debug("【Brush】发布时间不符合条件。")
                        return False

        except Exception as err:
            ExceptionUtils.exception_traceback(err)
            log.error("【Brush】%s 选种规则校验失败，已跳过该资源：%s" % (title, str(err)))
            return False
