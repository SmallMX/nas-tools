...
import log
from app.helper import ChromeHelper, DbHelper
from app.message import Message
from app.sites.sites import Sites
from app.sites.siteuserinfo import SITE_USER_INFO_HANDLERS
...
class SiteUserInfo(object):
    ...
    def __init__(self):
        self._site_schema = list(SITE_USER_INFO_HANDLERS)
    ...
    def build(self, url, site_name, site_cookie=None, ua=None, emulate=None, proxy=False):
        ...
        safe_url = StringUtils.redact_url(url)
        log.debug(f"【Sites】站点 {site_name} url={safe_url} "
                  f"cookie_configured={bool(site_cookie)} cookie_length={len(site_cookie)} ua={ua}")
            ...
            if res and res.status_code == 200:
                ...
                if html_text.find("title") == -1:
                    ...
                    if res and res.status_code == 200:
                        ...
                        log.error("【Sites】站点 %s 被反爬限制：%s, 状态码：%s" % (site_name, safe_url, res.status_code))
                        return None
                ...
                log.error(f"【Sites】站点 {site_name} 无法访问：{safe_url}")
                return None
        # 解析站点类型
